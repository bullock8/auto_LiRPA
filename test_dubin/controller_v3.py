...
class State:
    ...
    def __init__(self, rho, theta, theta2, psi, psi2, v_own, v_int, timer_DL, agent_mode: AgentMode):#__init__(self, x, y, theta, v, agent_mode: AgentMode):
        pass
...
def decisionLogic(ego: State, others: List[State]):
    ...
    if ego.timer_DL >= acas_update_time:
        ...
        if ego.agent_mode == AgentMode.COC:  # advisory 0
            if rho <= 15139.2:
                if theta <= -0.22 or theta2 <= -0.22:
                    if rho <= 1727.29:
                        if psi <= 0.22 or psi2 <= 0.22:
                            if rho <= 1320.87:
                                next.agent_mode = AgentMode.SL
                                next.timer_DL = 0
                            if rho > 1320.87:
                                if theta <= -1.73 or theta2 <= -1.73:
                    ...
                    if rho > 1727.29:
                        if theta <= -0.87 or theta2 <= -0.87:
                            if psi <= -0.22 or psi2 <= -0.22:
                                ...
                                if rho > 5385.08:
                                    ...
                                    next.timer_DL = 0
                            if psi > -0.22 or psi2 > -0.22:
                                if theta <= -2.82 or theta2 <= -2.82:
                                    if psi <= 0.43 or psi2 <= 0.43:
                                        next.agent_mode = AgentMode.WL
                                        next.timer_DL = 0
                                    if psi > 0.43 or psi2 > 0.43:
                                ...
                                if theta > -2.82 or theta2 > -2.82:
                                    if rho <= 6197.93:
                                        if psi <= 1.95 or psi2 <= 1.95:
                                            if theta <= -2.17 or theta2 <= -2.17:
                                                next.agent_mode = AgentMode.WL
                                                next.timer_DL = 0
                                            if theta > -2.17 or theta2 > -2.17:
                                                if rho <= 4369.03:
                        ...
                        if theta > -0.87 or theta2 > -0.87:
                            ...
                            if rho > 4369.03:
                                if rho <= 9652.51:
                                    if psi <= 1.08 or psi2 <= 1.08:
                                        next.agent_mode = AgentMode.WL
                                        next.timer_DL = 0
                                    if psi > 1.08 or psi2 > 1.08:
                                        if theta <= -0.43 or theta2 <= -0.43:
                                            next.agent_mode = AgentMode.SL
                                            next.timer_DL = 0
                                        if theta > -0.43 or theta2 > -0.43:
                                            if psi <= 2.6 or psi2 <= 2.6:
                                                next.agent_mode = AgentMode.SR
                                                next.timer_DL = 0
                                            if psi > 2.6 or psi2 > 2.6:
                                                next.agent_mode = AgentMode.SL
                            ...
                            if psi <= 0.0 or psi2 <= 0.0:
                                ...
                                if theta > 0.43 or theta2 > 0.43:
                                    ...
                                    if rho > 1320.87:
                                        ...
                                        next.timer_DL = 0
                            if psi > 0.0 or psi2 > 0.0:
                                if theta <= 1.08 or theta2 <= 1.08:
                                    if theta <= -0.22 or theta2 <= -0.22:
                                        if psi <= 2.38 or psi2 <= 2.38:
                                            next.agent_mode = AgentMode.SR
                                            next.timer_DL = 0
                                        if psi > 2.38 or psi2 > 2.38:
                                            next.agent_mode = AgentMode.SL
